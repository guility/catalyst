...
from catalyst.rl.core import AlgorithmSpec, ActorSpec, CriticSpec, EnvironmentSpec
from catalyst.rl import utils


class ACE(OffpolicyActorCritic):
    """
    Actor-Critic Ensemble extended algorithm from https://arxiv.org/abs/1712.08987.
    """
    ...
    def _init(
        ...
        self._actor_train = actor_train

        actors = [x.to(self._device) for x in actors]
        critics = [x.to(self._device) for x in critics]
        target_actors = [copy.deepcopy(x).to(self._device) for x in actors]
        ...
        for actor in actors:
            actor_components = utils.get_trainer_components(
                agent=actor,
                loss_params=self._actor_loss_params,
                optimizer_params=self._actor_optimizer_params,
                scheduler_params=self._actor_scheduler_params,
                grad_clip_params=self._actor_grad_clip_params
            )
            actors_optimizer.append(actor_components["optimizer"])
            actors_scheduler.append(actor_components["scheduler"])
        ...
        self.actors = [self.actor] + actors
        self.actors_optimizer = [self.actor_optimizer] + actors_optimizer
        self.actors_scheduler = [self.actor_scheduler] + actors_scheduler
        self.target_actors = [self.target_actor] + target_actors
    ...
    def _base_loss(self, states_t, actions_t, rewards_t, states_tp1, done_t):
        ...
        for actor in self.actors:
            ...
            actor_loss = -torch.mean(q_values_tp0_min)
    ...
    def actor_update(self, loss):
        ...
        for i in range(len(self.actors)):
            ...
            if self.actor_grad_clip_fn is not None:
                self.actor_grad_clip_fn(self.actors[i].parameters())
            self.actors_optimizer[i].step()
            if self.actors_scheduler[i] is not None:
                self.actors_scheduler[i].step()
                lr = self.actors_scheduler[i].get_lr()[0]
                metrics[f"lr_actor/{i}"] = lr
        return metrics

    def target_actor_update(self):
        for target, source in zip(self.target_actors, self.actors):
            utils.soft_update(target, source, self._actor_tau)
    ...
    def update_step(
        ...
        for i, x in enumerate(policy_loss):
            metrics[f"loss_actor/{i}"] = x.item()
        metrics = {
            **{
                "loss/common": loss.item(),
                "loss/actors": sum(policy_loss).item(),
                "loss/critics": sum(value_loss).item()
            },
            **metrics
        }
    ...
    def prepare_for_trainer(
        ...
        num_actors = config_["algorithm"].pop("num_actors", 3)
        num_critics = config_["algorithm"].pop("num_critics", 3)
